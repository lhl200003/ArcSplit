...
Live demo: https://arcsplit.testnetwork.workers.dev
...
```
Tests
Contract tests live in `contracts/test/ArcSplit.t.sol` and do not require `forge-std`.
```bash
cd contracts
forge test -vv
```
Covered cases: proportional allocation, remainder dust, accumulated deposits, double claim, non-recipient claim, zero deposit, recipient count limits, zero/duplicate recipients, shares not equal to 100%, and factory owner indexing.
The frontend converts percentages to basis points in `src/lib/shares.ts` and will not submit a split unless the resulting BPS total is exactly `10000`.
Status
